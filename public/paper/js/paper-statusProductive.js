statusProductive = {
    index: function() {
        const tableStatusProductive = $('#tableStatusProductive').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
            ...
            ajax: {
                url: "/statusProductive/data", // Laravel route
                type: "GET",
                dataSrc: "data"
            },
            columns: [
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-whiskey-glass text-success"></i> Registrar Estado';
            document.querySelector('#id').value = '';

            const containerRegister = document.getElementById('containerRegister');
            containerRegister.innerHTML = '';
            
            formStatusProductive.reset();
        });

    },

    form: function() {
        if (document.querySelector("#formStatusProductive")) {
            const formStatusProductive = document.querySelector("#formStatusProductive");

            formStatusProductive.onsubmit = function (e) {
                e.preventDefault();
                ...
                if(id == ''){
                    var url = '/statusProductive/create'
                    var metodo = 'POST';
                }else{
                    var url = '/statusProductive/update'
                    var metodo = 'POST';
                }
                ...
                const formData = new FormData(formStatusProductive);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Estado Productivo", data.msg, "success").then(() => {
                                $('#tableStatusProductive').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editStatusProductive($id){
            
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url+'/statusProductive/getStatusProductive/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-whiskey-glass text-success"></i> Actualizar Estado';
