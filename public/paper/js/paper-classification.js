classification = {
    index: function() {
        const tableClassification = $('#tableClassification').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
            ...
            ajax: {
                url: "/classification/data", // Laravel route
                type: "GET",
                dataSrc: "data"
            },
            columns: [
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-grip text-success"></i> Registrar Clasificación';
            document.querySelector('#id').value = '';

            const containerRegister = document.getElementById('containerRegister');
            containerRegister.innerHTML = '';
            
            formClassification.reset();
        });

    },

    form: function() {
        if (document.querySelector("#formClassification")) {
            const formClassification = document.querySelector("#formClassification");

            formClassification.onsubmit = function (e) {
                e.preventDefault();
                ...
                if(id == ''){
                    var url = '/classification/create'
                    var metodo = 'POST';
                }else{
                    var url = '/classification/update'
                    var metodo = 'POST';
                }
                ...
                const formData = new FormData(formClassification);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Clasificación", data.msg, "success").then(() => {
                                $('#tableClassification').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editClassification($id){
    ...
    let ajaxUrl = base_url+'/classification/getClassification/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-grip text-success"></i> Actualizar Clasificación';
