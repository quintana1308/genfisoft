guide = {
    index: function() {
        const tableGuide = $('#tableGuide').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-book text-success"></i> Registrar Guía';
    ...
    form: function() {
        if (document.querySelector("#formGuide")) {
            const formGuide = document.querySelector("#formGuide");

            formGuide.onsubmit = function (e) {
                e.preventDefault();
                ...
                const formData = new FormData(formGuide);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Guía", data.msg, "success").then(() => {
                                $('#tableGuide').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editGuide($id){
    ...
    let ajaxUrl = base_url+'/guide/getGuide/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-book text-success"></i> Actualizar Guía';
