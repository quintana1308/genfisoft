color = {
    index: function() {
        const tableColor = $('#tableColor').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-droplet text-success"></i> Registrar Color';
    ...
    form: function() {
        if (document.querySelector("#formColor")) {
            ...
            formColor.onsubmit = function (e) {
                ...
                const formData = new FormData(formColor);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Color", data.msg, "success").then(() => {
                                $('#tableColor').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editColor($id){
    ...
    let ajaxUrl = base_url+'/color/getColor/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-droplet text-success"></i> Actualizar Color';
