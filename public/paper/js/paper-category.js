category = {
    index: function() {
        const tableCategory = $('#tableCategory').DataTable({
            ...
            ajax: {
                url: "/category/data", // Laravel route
                type: "GET",
                dataSrc: "data"
            },
            columns: [
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-layer-group text-success"></i> Registrar Categoría';
    ...
    form: function() {
        if (document.querySelector("#formCategory")) {
            ...
            formCategory.onsubmit = function (e) {
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Categoría", data.msg, "success").then(() => {
                                $('#tableCategory').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editCategory($id){
    ...
    let ajaxUrl = base_url+'/category/getCategory/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-layer-group text-success"></i> Actualizar Categoría';
