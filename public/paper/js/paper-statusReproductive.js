statusReproductive = {
    index: function() {
        const tableStatusReproductive = $('#tableStatusReproductive').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
            ...
            ajax: {
                url: "/statusReproductive/data", // Laravel route
                type: "GET",
                dataSrc: "data"
            },
            columns: [
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-chart-column text-success"></i> Registrar Estado';
            document.querySelector('#id').value = '';

            const containerRegister = document.getElementById('containerRegister');
            containerRegister.innerHTML = '';
            
            formStatusReproductive.reset();
        });

    },

    form: function() {
        if (document.querySelector("#formStatusReproductive")) {
            const formStatusReproductive = document.querySelector("#formStatusReproductive");

            formStatusReproductive.onsubmit = function (e) {
                e.preventDefault();
                ...
                if(id == ''){
                    var url = '/statusReproductive/create'
                    var metodo = 'POST';
                }else{
                    var url = '/statusReproductive/update'
                    var metodo = 'POST';
                }
                ...
                const formData = new FormData(formStatusReproductive);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Estado Reproductivo", data.msg, "success").then(() => {
                                $('#tableStatusReproductive').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editStatusReproductive($id){
            
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url+'/statusReproductive/getStatusReproductive/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-chart-column text-success"></i> Actualizar Estado';
