owner = {
    index: function() {
        const tableOwner = $('#tableOwner').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-user-tie text-success"></i> Registrar Propietario';
    ...
    form: function() {
        if (document.querySelector("#formOwner")) {
            const formOwner = document.querySelector("#formOwner");

            formOwner.onsubmit = function (e) {
                e.preventDefault();
                ...
                const formData = new FormData(formOwner);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Propietario", data.msg, "success").then(() => {
                                $('#tableOwner').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editOwner($id){
    ...
    let ajaxUrl = base_url+'/owner/getOwner/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-user-tie text-success"></i> Actualizar Propietario';
