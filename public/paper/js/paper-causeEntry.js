causeEntry = {
    index: function() {
        const tableCauseEntry = $('#tableCauseEntry').DataTable({
            dom: 'lBfrtip',
            processing: true,
            serverSide: true,
            paging: false,
            ...
            ajax: {
                url: "/causeEntry/data", // Laravel route
                type: "GET",
                dataSrc: "data"
            },
            columns: [
        ...
        document.getElementById('containerRegister').addEventListener('click', function() {
            ...
            document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-scroll text-success"></i> Registrar Causa';
            document.querySelector('#id').value = '';

            const containerRegister = document.getElementById('containerRegister');
            containerRegister.innerHTML = '';
            
            formCauseEntry.reset();
        });
    },

    form: function() {
        if (document.querySelector("#formCauseEntry")) {
            const formCauseEntry = document.querySelector("#formCauseEntry");

            formCauseEntry.onsubmit = function (e) {
                e.preventDefault();
                ...
                if(id == ''){
                    var url = '/causeEntry/create'
                    var metodo = 'POST';
                }else{
                    var url = '/causeEntry/update'
                    var metodo = 'POST';
                }
                ...
                const formData = new FormData(formCauseEntry);
                ...
                fetch(url, {
                    ...
                    .then(data => {
                        ...
                        if (data.status) {
                            Swal.fire("Causa de entrada", data.msg, "success").then(() => {
                                $('#tableCauseEntry').DataTable().ajax.reload(); // ✅ recarga el DataTable por ajax
...
function editCauseEntry($id){
            
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url+'/causeEntry/getCauseEntry/'+$id;
    ...
    request.onreadystatechange = function(){
        ...
        if(request.readyState == 4 && request.status == 200){
            ...
            if(objData.status)
            {
                ...
                document.querySelector('#titleHeaderForm').innerHTML = '<i class="fa-solid fa-scroll text-success"></i> Actualizar Causa';
